...
@endsection
@section('title', 'Shoe-tik | Product Details')
                        ...
                        <div class="col-lg-6 col-12">
                            <div class="product-des">
                                ...
                                <div class="short">
                                    ...
                                    @endphp
                                    <p class="price"><span
                                            class="discount">Rp.{{ number_format($after_discount, 2, ',', '.') }}</span><s>Rp.
                                            {{ number_format($product_detail->price, 2, ',', '.') }}</s>
                                    </p>
                                ...
                                <div class="product-buy">
                                    <form action="{{ route('single-add-to-cart') }}" method="POST"
                                        ...
                                        <div class="warp-size">
                                            ...
                                            <select name="size" id="size" class="form-control">
                                                ...
                                                @foreach (explode(',', $product_detail->size) as $size)
                                                    <option value="{{ $size }}"
                                                        {{ old('size', $selectedSize) == $size ? 'selected' : '' }}>
                                                        {{ $size }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="warp-quantity">
                                            <div class="quantity">
                                                <h6>Quantity :</h6>
                                                <div class="qty">
                                    ...
                                    <p class="availability"> Stock:
                                        @if ($product_detail->stock > 0)
                                            @if ($product_detail->stock < 5)
                                                <span class="badge badge-warning">{{ $product_detail->stock }}</span>
                                            @else
...
@endpush
@push('scripts')
    <!-- Mengimpor SweetAlert -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/sweetalert/2.1.2/sweetalert.min.js"></script>
    <script>
        function validateQuantity() {
            const inputElement = document.getElementById('quantity');
            const max = parseInt(inputElement.getAttribute('data-max'), 10); // Mendapatkan nilai max dari data-max
            const min = parseInt(inputElement.getAttribute('data-min'), 10); // Mendapatkan nilai min dari data-min
            const value = parseInt(inputElement.value, 10); // Mendapatkan nilai input

            // Menghapus validasi default browser
            inputElement.setCustomValidity('');

            // Validasi jika nilai kurang dari minimum
            if (value < min) {
                // Menampilkan pesan error dengan SweetAlert
                swal("Error", `Jumlah tidak boleh kurang dari ${min}.`, "error");
                inputElement.setCustomValidity(`Jumlah tidak boleh kurang dari ${min}.`);
            }
            // Validasi jika nilai lebih besar dari stok
            else if (value > max) {
                // Menampilkan pesan error dengan SweetAlert
                swal("Error", `Jumlah tidak boleh lebih dari stok yang tersedia (${max}).`, "error");
                inputElement.setCustomValidity(`Jumlah tidak boleh lebih dari stok yang tersedia (${max}).`);
            }
            // Jika input valid
            else {
                // Menampilkan pesan sukses dengan SweetAlert
                swal("Berhasil", `Jumlah ${value} valid!`, "success");
                inputElement.setCustomValidity(''); // Reset custom validity jika valid
            }
        }

        // Menyesuaikan opsi Toastr (jika Anda ingin menggunakan Toastr)
        toastr.options = {
            "closeButton": true,
            "progressBar": true,
